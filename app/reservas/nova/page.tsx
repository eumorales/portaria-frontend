...
export default function NovaReservaPage() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true)
      const [itens, users] = await Promise.all([api.getAvailableItems(), api.getUsers()])
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-blue-100">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">Crie uma nova reserva selecionando um item e usuário</p>
        </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                      ...
                      {itensDisponiveis.map((item) => (
                        <SelectItem key={item.id} value={item.id}>
                          <div className="flex items-center gap-2">
                            <span>{item.nome}</span>
                            <Badge className={getTipoColor(item.tipo)} variant="secondary">
                              {item.tipo}
                            </Badge>
                          </div>
                        </SelectItem>
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setFormData({ ...formData, matriculaUsuario: e.target.value })}
                ...
                <div className="flex gap-4">
                  <Button type="button" variant="outline" onClick={() => router.push("/reservas")} className="flex-1">
                    Cancelar
                  </Button>
            ...
            <Card className="bg-white shadow-lg">
              ...
              <CardContent>
                {itensDisponiveis.length === 0 ? (
                  <p className="text-gray-500 text-center py-4">Nenhum item disponível no momento</p>
                ) : (
                  <div className="space-y-3 max-h-64 overflow-y-auto">
                    {itensDisponiveis.map((item) => (
                      <div
                        ...
                        onClick={() => setFormData({ ...formData, itemId: item.id })}
                      >
                        ...
                        <p className="text-sm text-gray-600 mt-1">{item.localizacao}</p>
                      </div>
                    ))}
                  </div>
                )}
                  ...
                  {usuarios.map((usuario) => (
                    ...
                    >
                      <div>
                        ...
                        <p className="text-sm text-gray-600">{usuario.matricula}</p>
                      </div>
                      <Badge className={getTipoUsuarioColor(usuario.tipo)} variant="secondary">
                        {usuario.tipo}
                      </Badge>
                    </div>
                  ))}
